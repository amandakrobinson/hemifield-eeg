function plot_Fig8_neuralbehaviouralcorr()
%% plot figure 8 of peripheral manuscript
...
t = {'Image task' 'Concept task' 'Difference'};
...
for plotnr = 1:length(ci)
    ...
    for m = 1:3 % two models then differences
        ...
        text(120,5e-3,t{m},'Fontsize',20,'Color',col)
...
xlim([-50 800])
ylim([-.05 .15])
mu = movmean(stats.Peripheral.L_elecs.RVF.mu(:,1),40);
plot(timevect,mu,'Color',cmap(1,:),'Linewidth',2)
mu = movmean(stats.Peripheral.L_elecs.RVF.mu(:,2),40);
plot(timevect,mu,'Color',cmap(2,:),'Linewidth',2)
xlabel('Time (ms)')
set(gca,'XTick',[])
set(gca,'YTick',[])
ylabel('Correlation')
...
fn = 'figures/Figure8_behaviouralRSA';
tn = tempname;
print(gcf,'-dpng','-r300',tn)
im=imread([tn '.png']);
