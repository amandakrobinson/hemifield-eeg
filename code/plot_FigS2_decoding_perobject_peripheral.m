...
% plot Figures S2: mean decoding accuracy per object class for peripheral stimuli

function plot_FigS2_decoding_perobject_peripheral()
...
%% load results
load('results/stats_decoding_peripheral_perobs.mat','stats');
timevect = stats.timevect;

%% set variables
conds = {'Peripheral' 'MultiplePeripheral'};
oconds = {'objects','faces','words'};
...
for cond = 1:length(conds) % peripheral, multi peripheral
    for o = 1:length(objs)
        %% initialise figure
        fig=figure((cond-1)*3+o);clf;
        fig.Resize = 'off';
        fig.Position=[1 1 2000 900];

        %% plot per condition
        dat_p = stats.(conds{cond}).(oconds{o});
        ...
        if cond==1
            fn = sprintf('figures/FigureS2.%d_decoding_%s_%s.png',o,conds{cond},oconds{o});
            print(gcf,'-dpng','-r500',fn)
            im=imread(fn);
            [i,j]=find(mean(im,3)<255);margin=2;
            imwrite(im(min(i-margin):max(i+margin),min(j-margin):max(j+margin),:),fn,'png');
        end
    end
end
